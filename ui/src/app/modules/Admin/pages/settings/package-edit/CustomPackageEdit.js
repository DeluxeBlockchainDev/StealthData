...
export function CustomPackageEdit({
  history,
  match: {
    params: { id },
  },
  ...
  useEffect(() => {
    let _title = id ? "" : "New Custom Package";
    if (subscriptionForEdit && id) {
      _title = `Edit Custom Package - ${subscriptionForEdit.name}'`;
  ...
  const backToList = ( ) => {
    ...
    history.push('/admin/settings/customPackages');
  }
